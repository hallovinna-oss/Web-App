 ...
 * Contains Firebase Auth + Firestore bridge for MIPHA Companion.
 */
...
const FIREBASE_ATTENDANCE_START = window.MIPHA_CONFIG?.attendanceStart || '2026-07-13';
...
const FirebaseBackend = {
  ...
  startListeners(appState) {
    ...
    if (appState.currentUser?.role === 'guru') {
      this.syncHistoricalAttendance(appState).catch((error) => {
        console.warn('Historical attendance sync failed:', error);
      });
    }
  },

  async syncHistoricalAttendance(appState) {
    if (!auth.currentUser) return;

    const start = FIREBASE_ATTENDANCE_START;
    ...
    snapshot.docs.forEach((doc) => {
      ...
      if (!appState.monthlyAttendance[data.studentId]) appState.monthlyAttendance[data.studentId] = {};
      appState.monthlyAttendance[data.studentId][data.date] = data.status || 'alpha';
      if (data.date === end) {
        appState.attendance[data.studentId] = { ...appState.attendance[data.studentId], ...data };
      }
    });
